...
% Features  = zeros(size(signal,1),1500);
%%
...
%     wndw = exp(-0.5*(sigma*(k - footfall_len/2)./(footfall_len/2)).^2); % gaussian window started at maximum point
    wndw = tukeywin(footfall_len,0.5); % turkey window

    
%     strt = mn - footfall_len/2-1;
...
%     Sig = w_diag*sig;
%     Sig(abs(Sig)<0.0005) = 0;
     evnt_len(iter,:) = sum(Sig ~=0);  
    
%     new_sig(strt:stop) = Sig;
%     Features(iter,:) = feature_extraction_v1(Sig,1);
...
% Features = [Features , prsn*ones(iter-1,1)];
